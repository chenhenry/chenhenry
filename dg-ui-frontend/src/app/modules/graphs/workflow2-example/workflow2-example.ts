...
var component = angular.module('app.modules.graphs.workflow2-example', [
	'app.components.graph',
	'app.modules.graphs.edit-node'
])
	.directive('workflow2Example', function() {
		return {
			replace: true,
			restrict: 'E',
			scope: {},
			...
			controller: function() {
				...
				ctrl.nodes = [
					{ id: 1, shape: 'ellipse', content: { text: 'CMM First Approval' }, template: nodeTemplate },
					{ id: 2, shape: 'rect', content: { text: 'CMM Second Approval' }, template: nodeTemplate },
					{ id: 3, shape: 'diamond', content: { text: 'CCMM Final Approval' }, template: nodeTemplate }
				];
				...
				// label = string
				ctrl.edges = [
					{ from: 1, to: 2, type: 'undirected', dashed: false },
					{ from: 2, to: 3, type: 'undirected', dashed: false }
				];
			...
			controllerAs: 'ctrl',
			template: require('./workflow2-example.html')
		};
	}).name;
